...
class Textures3dSamplerArrayMoreQuadsApp : public VulkanApplication3D
{
#include "..\..\models\quad.vertices"
	...
	{
		texture.LoadArray(system, VulkanPlayground::GetModelFile("Basics", "texturearray_bc3_unorm.ktx"), VK_FORMAT_BC3_UNORM_BLOCK);
		...
		Shader &shader = pipeline.LoadShader(system, "SamplerArrayMoreQuads");
		shader.SetSpecializationConstant(VK_SHADER_STAGE_VERTEX_BIT, 0, texture.GetNumLayers());
		shader.SetSpecializationConstant(VK_SHADER_STAGE_FRAGMENT_BIT, 0, texture.GetNumLayers());
		...
		pipeline.AddPushConstant(system, sizeof(multiSample), VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	{
		pipeline.PushConstant(commandBuffer, &multiSample);
...
DECLARE_APP(Textures3dSamplerArrayMoreQuads)
