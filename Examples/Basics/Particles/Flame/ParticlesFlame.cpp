...
class ParticlesFlameApp : public VulkanApplication3D
{
...
public:
	...
	{
		flame.SetAddressMode(VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER);	// Avoid edges bleeding over
		...
		pipeline.LoadShaderDiffNames(system, "Particles", "Flame");
		...
		particles.Prepare(system, testBigParticles ? 3 : PARTICLE_COUNT, testBigParticles ? 1 : 0);
	...
	void UpdateScene(VulkanSystem& system, float frameTime) override
	{
		...
		mvpUBO().model = glm::translate(mvpUBO().model, glm::vec3(0, 10, 0));
	}
