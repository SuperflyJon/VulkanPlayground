namespace gli
{
	...
	inline storage_linear::size_type storage_linear::image_offset(extent1d const& Coord, extent1d const& ExtentX) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, ExtentX)));
		return static_cast<size_t>(Coord.x);
	}

	inline storage_linear::size_type storage_linear::image_offset(extent2d const& Coord, extent2d const& ExtentX) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, ExtentX)));
		return static_cast<size_t>(Coord.x + Coord.y * ExtentX.x);
	}

	inline storage_linear::size_type storage_linear::image_offset(extent3d const& Coord, extent3d const& ExtentX) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, ExtentX)));
		return static_cast<storage_linear::size_type>(Coord.x + Coord.y * ExtentX.x + Coord.z * ExtentX.x * ExtentX.y);
	}
		...
		extent_type const& BlockCountX)
	{
		...
		for(size_t BlockIndexZ = 0, BlockCountZ = BlockCountX.z; BlockIndexZ < BlockCountZ; ++BlockIndexZ)
		for(size_t BlockIndexY = 0, BlockCountY = BlockCountX.y; BlockIndexY < BlockCountY; ++BlockIndexY)
		{
			...
			memcpy(DataDst, DataSrc, this->block_size() * BlockCountX.x);
		}
	}
