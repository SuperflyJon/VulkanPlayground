...
namespace detail
{
	...
	inline gl::format gl::translate(gli::format Format, gli::swizzles const& Swizzles) const
	{
		...
		gl::format_desc const& FormatDescX = this->FormatDesc[Format - FORMAT_FIRST];

		gl::format FormatGL;
		FormatGL.Internal = FormatDescX.Internal;
		FormatGL.External = FormatDescX.External;
		FormatGL.Type = FormatDescX.Type;
		FormatGL.Swizzles = this->compute_swizzle(FormatDescX, Swizzles);
		return FormatGL;
	}
	...
	inline gl::swizzles gl::compute_swizzle(format_desc const& FormatDescX, gli::swizzles const& Swizzles) const
	{
		if (!this->has_swizzle(this->Profile))
			return swizzles(gl::SWIZZLE_RED, gl::SWIZZLE_GREEN, gl::SWIZZLE_BLUE, gl::SWIZZLE_ALPHA);

		bool const IsExternalBGRA = ((FormatDescX.Properties & detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT) && !has_swizzle(this->Profile)) || (FormatDescX.Properties & detail::FORMAT_PROPERTY_BGRA_TYPE_BIT);
